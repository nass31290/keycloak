...
/**
 * Route GET pour afficher la page d'accueil de l'ue3.
 * 
 ...
 */
router.get('/', keycloak.enforcer(['ue3:read']), (req, res, next) => {
    try {
        // Trouvez les données de l'ue3 dans votre fichier JSON ou base de données
        const ue3 = ues.find(ue => ue.id === 'ue3');
        if (!ue3) {
            let error = new Error('ue3 non trouvée');
            error.status = 404;
            throw error;
        }
        res.render('ue', { ue: ue3 });        
    } catch (error) {
        next(error);
    }
});
...
/**
 * Route POST pour écrire/modifier une note d'un étudiant pour l'ue3.
 * 
 * @param {Object} req - L'objet de requête Express, contenant les informations de la requête.
 ...
 * @param {function} next - La fonction middleware suivante dans la pile Express.
 */
router.post('/write', keycloak.enforcer(['ue3:write']), (req, res, next) => {
    const studentName = req.body.studentName;
    const newNote = req.body.newNote;
    let ue = ues.find(ue => ue.id === 'ue3');
...
/**
 * Route POST pour valider la note d'un étudiant pour l'ue3.
 * 
 * @param {Object} req - L'objet de requête Express, contenant les informations de la requête.
 ...
 * @param {function} next - La fonction middleware suivante dans la pile Express.
 */
router.post('/validate', keycloak.enforcer(['ue3:validate']), (req, res, next) => {
    const studentName = req.body.studentName;
    let ue = ues.find(ue => ue.id === 'ue3');
    ...
    // Rediriger l'utilisateur vers la page /ue3 après la validation
    res.redirect('/ue3');
    res.status(200).end('success');
});

/**
 * Route POST pour supprimer la note d'un étudiant pour l'ue3.
 * 
 * @param {Object} req - L'objet de requête Express, contenant les informations de la requête.
 ...
 * @param {function} next - La fonction middleware suivante dans la pile Express.
 */
router.post('/delete', keycloak.enforcer(['ue3:write']), (req, res, next) => {
    const studentName = req.body.studentName;
    let ue = ues.find(ue => ue.id === 'ue3');
